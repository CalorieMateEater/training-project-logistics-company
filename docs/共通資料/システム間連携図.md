...
flowchart LR
    ...
    subgraph aws["Hoge社AWS (ap-northeast-1)"]
        subgraph vpc_hoge["連携・業務VPC"]
            ...
            subgraph hoge_g2[" "]
                ...
                H_WORKER@{img: "https://api.iconify.design/logos/aws-ecs.svg", label: "出荷・配送管理ハブ<br/>Bar連携Worker", pos: "b", w: 54, h: 54, constraint: "on"}
                ...
                H_REQ_API@{img: "https://api.iconify.design/material-symbols/api.svg", label: "配送連携API<br/>出荷依頼受付API", pos: "b", w: 54, h: 54, constraint: "on"}
                H_STATUS_API@{img: "https://api.iconify.design/material-symbols/api.svg", label: "配送連携API<br/>出荷状態照会API", pos: "b", w: 54, h: 54, constraint: "on"}
            end
            subgraph hoge_g3[" "]
                ...
                H_STOCK@{img: "https://api.iconify.design/logos/aws-ecs.svg", label: "在庫引当管理", pos: "b", w: 54, h: 54, constraint: "on"}
    ...
    subgraph FUGA_NET["Fuga社業務ネットワーク"]
        subgraph fuga_g1[" "]
            FUGA_REQ_CLIENT@{img: "https://api.iconify.design/material-symbols/dns.svg", label: "出荷依頼クライアント", pos: "b", w: 54, h: 54, constraint: "on"}
            FUGA_STATUS_CLIENT@{img: "https://api.iconify.design/material-symbols/dns.svg", label: "状態照会クライアント", pos: "b", w: 54, h: 54, constraint: "on"}
            FUGA_REQ_GATE@{img: "https://api.iconify.design/material-symbols/api.svg", label: "Fuga API送信<br/>出荷依頼", pos: "b", w: 54, h: 54, constraint: "on"}
            FUGA_STATUS_GATE@{img: "https://api.iconify.design/material-symbols/api.svg", label: "Fuga API送信<br/>状態照会", pos: "b", w: 54, h: 54, constraint: "on"}
            FUGA_EDGE@{img: "https://api.iconify.design/material-symbols/security.svg", label: "Fuga社接続境界", pos: "b", w: 54, h: 54, constraint: "on"}
            FUGA_SYS@{img: "https://api.iconify.design/material-symbols/domain.svg", label: "出荷依頼管理システム", pos: "b", w: 54, h: 54, constraint: "on"}
        end
    end
    ...
    FUGA_SYS --> FUGA_REQ_CLIENT
    FUGA_SYS --> FUGA_STATUS_CLIENT
    FUGA_REQ_CLIENT -->|出荷依頼API| FUGA_REQ_GATE
    FUGA_REQ_GATE --> FUGA_EDGE
    FUGA_STATUS_CLIENT -->|状態照会HTTPS| FUGA_STATUS_GATE
    FUGA_STATUS_GATE --> FUGA_EDGE
    FUGA_EDGE -->|VPN閉域連携| H_FUGA_EDGE
    H_FUGA_EDGE -->|出荷依頼API入口| H_REQ_API
    H_FUGA_EDGE -->|状態照会HTTPS入口| H_STATUS_API
    H_REQ_API -->|顧客確認 / 在庫引当| H_API_GATE
    H_REQ_API -->|出荷依頼登録| H_STORE
    H_REQ_API -->|配送条件判定依頼| H_ROUTE
...
| 入力 | Foo社出荷依頼ファイル |
| 正常時処理 | ファイル受信確認、レコード読込、バリデーション、DB登録、Bar送信待ちキュー投入、後続連携 |
...
flowchart TD
    ...
    C1 --> D1["Internal API Gateway経由で在庫引当API呼び出し"]
    D1 --> R1["配送条件判定"]
    R1 --> DB1["出荷・配送状態データストア登録"]
    DB1 --> Q1["bar-shipment-request-queue.fifoへ投入"]
    Q1 --> W1["Bar連携Workerが取得"]
    W1 --> T1{"平日08:00-18:00?"}
    T1 -->|Yes| B1["Bar向け出荷指示API呼び出し"]
    T1 -->|No| Q2["次回営業時間まで待機"]
    B1 --> B2["billing-plan-queueへ投入"]
    B2 --> B3["order-notice-queue.fifoへ投入"]
    B3 --> E["正常終了"]
```
...
- Hoge社は `出荷依頼受付`、`配送条件判定`、`Bar送信待ちキュー制御`、`Bar連携`、`配送状態管理` を担う
- Bar社は `実配送` と `配送結果通知` を担う
- `Fuga社 出荷依頼クライアント` から `Fuga API送信 出荷依頼`、`Hoge 出荷依頼受付API` を通る線が、出荷依頼の主経路である
- `Fuga社 状態照会クライアント` から `Hoge 出荷状態照会API` への線が、状態照会の主経路である
- `Foo社 状態照会クライアント` と `Fuga社 状態照会クライアント` は、いずれも閉域VPN上の HTTPS で `Hoge 出荷状態照会API` を呼び出す
- 実際の受信点は `Hoge HULFT受信サーバ` と `ファイル着信S3` である
- Foo社からのファイル受信経路、Foo社/Fuga社からのAPI呼出経路、Hoge社からBar社への配送依頼経路は、別の接続経路として見分ける
- `Fuga社接続境界` は `Hoge社 Fuga向け接続境界` と直接つながっており、`Bar Delivery Center` とは直接連携しない
- `Hoge 出荷依頼受付API` は、受け付けた依頼を `出荷・配送状態データストア` へ登録した上で `配送条件判定` と `bar-shipment-request-queue.fifo` へ引き渡す
- `配送連携API` は `出荷依頼受付API` と `出荷状態照会API` を分けて表現している
- Bar社から返却された配送結果は、Hoge社内の `配送状態取込Worker` が受け取り、データストアへ反映する
...
本資料では、`Foo Order Gateway`、`Bar Delivery Center`、`Fuga Shipment Portal` の内部シーケンスは扱わない。  
