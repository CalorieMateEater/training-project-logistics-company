...
## 4. 設定値
| 項目 | `bar-shipment-request-queue.fifo` | `billing-plan-queue` | `order-notice-queue.fifo` |
| --- | --- | --- | --- |
| MessageRetentionPeriod | 7日 | 4日 | 4日 |
| VisibilityTimeout | 300秒 | 120秒 | 180秒 |
| ReceiveMessageWaitTimeSeconds | 20秒 | 20秒 | 20秒 |
| MaximumMessageSize | 256KB | 256KB | 256KB |
| 暗号化 | SSE-KMS | SSE-KMS | SSE-KMS |
| KMS キー | `alias/hoge-orderhub-sqs` | `alias/hoge-orderhub-sqs` | `alias/hoge-orderhub-sqs` |
| MessageGroupId | `carrier_code + business_date` | - | `tenant_code + order_id` |
| DeduplicationId | `shipment_request_id + request_version` | - | `event_id` |

## 5. 性能設計
| 観点 | `bar-shipment-request-queue.fifo` | `billing-plan-queue` | `order-notice-queue.fifo` |
| --- | --- | --- | --- |
| 想定投入量 | 1日 60,000件 | 1日 50,000件 | 1日 80,000件 |
| ピーク投入 | 20 TPS | 10 TPS | 15 TPS |
| Consumer 数 | Worker 2 タスク | Baz側 2 プロセス | Qux側 2 プロセス |
| 順序要件 | 同一配送会社・営業日単位で順序保持 | なし | 注文単位で順序保持 |
