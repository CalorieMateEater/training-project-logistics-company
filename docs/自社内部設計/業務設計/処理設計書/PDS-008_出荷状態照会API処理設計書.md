...
| 3 | Foo社は `partner_order_id` のみ、Hoge社内利用者は `partner_order_id` または `partner_request_id` の利用可、の利用制約を判定する |
...
- Hoge社内利用者から `partner_request_id` 指定時は、対象注文を一意に解決できる場合のみ返却する。
