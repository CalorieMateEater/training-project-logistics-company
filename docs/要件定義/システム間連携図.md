...
flowchart LR
    ...
    subgraph aws["Hoge社AWS (ap-northeast-1)"]
        subgraph vpc_hoge["連携・業務VPC"]
            subgraph hoge_g1[" "]
                H_HOGE_CLIENT@{img: "https://api.iconify.design/material-symbols/desktop-windows.svg", label: "Hoge社業務端末", pos: "b", w: 54, h: 54, constraint: "on"}
                H_WH_CLIENT@{img: "https://api.iconify.design/material-symbols/warehouse.svg", label: "倉庫担当者端末", pos: "b", w: 54, h: 54, constraint: "on"}
            ...
            subgraph hoge_g2[" "]
                ...
                H_ACK_OUT@{img: "https://api.iconify.design/logos/aws-s3.svg", label: "注文受付通知<br/>ファイル出力S3", pos: "b", w: 54, h: 54, constraint: "on"}
    ...
    H_BATCH -->|注文受付通知ファイル出力| H_ACK_OUT
    ...
    H_ACK_OUT --> H_HULFT_TX
    H_HULFT_TX -->|注文受付通知| H_FOO_EDGE

    H_HOGE_CLIENT -->|直受注登録HTTPS| H_STATUS_API
    H_WH_CLIENT -->|在庫照会 / 入庫登録HTTPS| H_API_GATE
    H_FOO_EDGE -->|状態照会HTTPS入口| H_STATUS_API
    H_STATUS_API -->|配送状態参照| H_STORE
    H_STATUS_API -->|直受注登録| H_STORE
    H_STATUS_API -->|配送条件判定依頼| H_ROUTE
...
| 入力 | Foo社出荷依頼ファイル |
| 正常時処理 | ファイル受信確認、レコード読込、バリデーション、DB登録、注文受付通知ファイル出力、配送条件判定、Bar/Fuga送信待ちキュー投入、後続連携 |
...
flowchart TD
    ...
    F2A --> E["正常終了"]
    B1A --> N1["billing-plan-queueへ仮請求投入"]
    N1 --> N2["Foo注文のみorder-notice-queue.fifoへ投入"]
    N2 --> E["正常終了"]
```

### 3.3 配送連携・在庫運用の利用イメージ
...
flowchart TD
    ...
    A5 --> A7["配送会社連携Worker"]
    A6 --> A7
    C1["Hoge社 業務端末"] -->|HTTPS| C2["Hoge 直受注登録API"]
    C2 --> A3
    D1["倉庫担当者端末"] -->|HTTPS| D2["Hoge 在庫照会 / 入庫登録API"]
    D2 --> D3["Hoge 在庫管理"]
...
- Foo社への `注文受付通知` は、配送会社への送信可否とは独立して、Hoge社の受付完了時点でHULFT返却する
- Fuga社は `特殊配送受託` と `配送結果通知` を行う
- Hoge社は `出荷依頼受付`、`配送条件判定`、`Bar/Fuga送信制御`、`配送状態管理` を担う
- Bar社は `標準配送` と `配送結果通知` を担う
- Hoge社業務部門からの `直受注登録` は `配送連携API` を経由して受け付ける
- Hoge社倉庫担当者は `Internal API Gateway` 経由で `在庫管理` の在庫照会 / 入庫登録APIを利用する
