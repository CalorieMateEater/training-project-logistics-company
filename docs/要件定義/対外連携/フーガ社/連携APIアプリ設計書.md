...
- `status_seq` が既処理値と同じで受信内容も同一の場合は、同一再送として状態反映要求を重複起票せず`202 Accepted`を返す
- `status_seq` が既処理値と同じで受信内容が異なる場合は、競合として`409 Conflict`を返す
- `status_seq` が既処理値より小さい場合は、旧イベントとして状態反映要求を起票せず`409 Conflict`を返す
- `status_seq` が既処理値より大きく状態遷移も妥当な場合は、状態反映要求を起票して`202 Accepted`を返す
- `delivery_status` の遷移不正は`409 Conflict`とする
...
| 特殊配送結果通知API | 409 | 状態遷移不正、同一連番の内容相違、または古い`status_seq` |
