...
class PriorityResolverTest {
  ...
  void shouldResolvePriorityForReservedOrder() {
    assertThat(priorityResolver.resolveFooPriority(OrderType.RESERVED, 8))
        .isEqualTo(ShippingPriorityClass.PRIORITY);
  }

  @Test
  void shouldResolveNormalForNonReservedOrder() {
    assertThat(priorityResolver.resolveFooPriority(OrderType.NORMAL, 8))
        .isEqualTo(ShippingPriorityClass.NORMAL);
  }
}
