...
/**
 * Shipping Gateway API の例外応答を統一するハンドラー。
...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleStatus(ResponseStatusException exception) {
        ...
        log.error("MONITORING_API_ERROR status={} message={}", status.value(), exception.getReason(), exception);
    ...
    public ResponseEntity<ApiErrorResponse> handleValidation(MethodArgumentNotValidException exception) {
        ...
        return ResponseEntity.badRequest()
                .body(new ApiErrorResponse("HSG-REQ-001", message));
    }
