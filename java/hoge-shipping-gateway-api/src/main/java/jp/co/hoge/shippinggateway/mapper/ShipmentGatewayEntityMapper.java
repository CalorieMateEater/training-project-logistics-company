...
@Mapper(config = CommonMapperConfig.class, imports = {
        CarrierCode.class,
        OrderSource.class,
        ShippingPriorityClass.class
})
public interface ShipmentGatewayEntityMapper {
    /**
     * Fuga出荷依頼受付情報から注文ヘッダを生成する。
     *
    ...
    @Mapping(target = "orderSource", expression = "java(OrderSource.FUGA)")
    ...
    OrderHeaderEntity toOrderHeader(FugaShipmentRegistrationContext source);

    /**
     * Fuga出荷依頼受付情報から注文明細を生成する。
     *
     * @param source 出荷依頼受付情報
     * @return 注文明細
     */
    ...
    @Mapping(target = "itemName", constant = "FugaRequestedItem")
    OrderLineEntity toOrderLine(FugaShipmentRegistrationContext source);

    /**
     * Fuga出荷依頼受付情報から顧客確認結果を生成する。
     *
     * @param source 出荷依頼受付情報
     * @return 顧客確認結果
     */
    ...
    CustomerCheckResultEntity toCustomerCheckResult(FugaShipmentRegistrationContext source);

    /**
     * Fuga出荷依頼受付情報から在庫引当結果を生成する。
     *
     * @param source 出荷依頼受付情報
     * @return 在庫引当結果
     */
    ...
    @Mapping(target = "reservedQuantity", source = "request.quantity")
    StockReservationResultEntity toStockReservationResult(FugaShipmentRegistrationContext source);

    /**
     * Fuga出荷依頼受付情報から出荷依頼を生成する。
     *
     * @param source 出荷依頼受付情報
     * @return 出荷依頼
     */
    ...
    @Mapping(target = "partnerPriorityLevel", constant = "0")
    @Mapping(target = "shippingPriorityClass", expression = "java(ShippingPriorityClass.NORMAL)")
    @Mapping(target = "routingRuleId", constant = "RULE-BAR-DEFAULT")
    @Mapping(target = "queueEnqueuedAt", source = "now")
    ShipmentRequestEntity toShipmentRequest(FugaShipmentRegistrationContext source);
    ...
    void updateDeliveryStatusCurrentEntity(
            BarDeliveryResultContext source,
            @MappingTarget DeliveryStatusCurrentEntity target);
}
